...
'''daily bias correction by cdf'''


'''find average min/max difference by month'''
# strip out day of month
mindifference['month'] = mindifference.index.map(lambda x: x[5:7])
monthly = mindifference.groupby('month')
minbymonthmu = monthly.mean()
minbymonthsd = monthly.std()
minupper = minbymonthmu+minbymonthsd
minlower = minbymonthmu-minbymonthsd

maxdifference['month'] = maxdifference.index.map(lambda x: x[5:7])
monthly = maxdifference.groupby('month')
maxbymonthmu = monthly.mean()
maxbymonthsd = monthly.std()
maxupper = maxbymonthmu+maxbymonthsd
maxlower = maxbymonthmu-maxbymonthsd

ax = minbymonthmu.plot(title='mean monthly difference', color='b')
minupper.plot(ax=ax, color='b')
minlower.plot(ax=ax, color='b')

#ax = maxbymonthmu.plot(title='mean monthly difference')
maxbymonthmu.plot(ax=ax, color='r')
maxupper.plot(ax=ax, color='r')
maxlower.plot(ax=ax, color='r')